...
```{r}
mydata<-read_excel("Assignment3HW3_Data.xlsx", sheet = 4)
...
#Based on the model results, customer joining via campaign or those joining organically has no significantly affect on the CLV
...
# ```{r}
# # churned_camp = data.frame(subset(preddata,preddata$Predicted_churn == 1 &  preddata$Camp_Org == 1 ))
# ret_camp = data.frame(subset(preddata,preddata$Camp_Org == 1 ))
# total_camp = data.frame(subset(preddata,  preddata$Camp_Org == 1 ))
# # 
# # churn_rate_camp =  nrow(churned_camp)/ nrow(total_camp)
# 
# r_camp = mean(ret_camp$Retention.rate)
# 
# #r_camp = 1 - churn_rate_camp ## retention rate of those who joined via campaign
# L_camp = 1/ (1-r_camp) ## avg expected customer life time who joined via campaign
# m_camp = mean(total_camp$Spend*3*0.50)
# CLV_camp <- m_camp*L_camp ## CLV of customers who joined via campaign
# CLV_camp
# 
# #churned_org = data.frame(subset(preddata,preddata$Predicted_churn == 1 &  preddata$Camp_Org == 0 ))
# total_org = data.frame(subset(preddata,  preddata$Camp_Org == 0 ))
# 
# #churn_rate_org =  nrow(churned_org)/ nrow(total_org)
#  ## retention rate of those who joined organically
# ret_org = data.frame(subset(preddata,preddata$Camp_Org == 0 ))
# r_org = mean(ret_org$Retention.rate)
# 
# 
# L_org = 1/ (1 - r_org) ## avg expected customer life time who joined organically
# m_org = mean(total_org$Spend*3*0.50)
# CLV_org <- m_org*L_org ## CLV of customers who joined organically
# CLV_org
# 
# #CLV and retention rate of customers who joined organically is higher than those customers who joined via campaign
# 
# sprintf("r_camp: %f",r_camp)
# sprintf("r_org: %f",r_org)
# sprintf("CLV_camp: %f",CLV_camp)
...
# 
# 
# ###### CLV for every customer (irrespective of campaign or not)
# 
# R_overall = mean(preddata$`Retention rate`)
# L_overall = 1/(1-R_overall)
# m_overall = mean(preddata$Spend*3)*0.50
# CLV_overall <- m_overall*L_overall ## CLV of customers
# CLV_overall
# 
...
# summary (CLV_Camp_org)
# 
# #Customers who joined via campaign or organically don't significantly influence CLV
```